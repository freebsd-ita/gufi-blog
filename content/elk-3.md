...
# the first time it will fail due to an error related to argparse
pip install -r requirements.txt
...
Create an alert (frequency based) that will send an email if there are more than 9 events in 1 hour with status: 404 and type: nginx
...
Removing the --debug, we start receiving e-mails. (yay!)
